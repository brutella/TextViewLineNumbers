...
class LineNumberRulerView: NSRulerView {
    ...
    override func drawHashMarksAndLabelsInRect(rect: NSRect) {
        ...
        if let context = NSGraphicsContext.currentContext()?.CGContext {
            ...
            CTFontGetAdvancesForGlyphs(ctFont, CTFontOrientation.OrientationHorizontal, &digitGlyphs[8], &advance, 1)
            ...
            lineNum += regex.numberOfMatchesInString(text, options: NSMatchingOptions.allZeros, range: NSMakeRange(0, characterCount))
            ...
            for glyphIndex = glyphCount; glyphIndex < NSMaxRange(visibleGlyphRange); lineIndex++ {
                ...
                while (glyphCount < glyphIndex) { // handle wrapper lines
                    ...
                    if lastLineNum == lineNum { // draw wrapped mark
                        ...
                        var glyphs = Array(defaultValue: CGGlyph(kCGGlyphMax), count: digit)
                        var positions = Array(defaultValue: CGPointZero, count: digit)
                        for i in 0..<digit {
                            let glyph = digitGlyphs[NumberAt(i, lineNum)]
                            glyphs[i] = glyph
